...
def index():
    ...
    cur.execute("SELECT * FROM products WHERE category=%s ORDER BY RAND() LIMIT 4", (values,))
    xbox = cur.fetchall()
    values = 'videojuegops4'
    cur.execute("SELECT * FROM products WHERE category=%s ORDER BY RAND() LIMIT 4", (values,))
    videojuegops4 = cur.fetchall()
    values = 'Videojuegoxbox'
    cur.execute("SELECT * FROM products WHERE category=%s ORDER BY RAND() LIMIT 4", (values,))
    Videojuegoxbox = cur.fetchall()
    # Close Connection
    cur.close()
    return render_template('home.html', play=play, xbox=xbox, videojuegops4=videojuegops4, Videojuegoxbox=Videojuegoxbox, form=form)
...
class RegisterForm(Form):
    ...
    password = PasswordField('', [validators.length(min=3)],
                             render_kw={'placeholder': 'Contraseña'})
    mobile = StringField('', [validators.length(min=11, max=15)], render_kw={'placeholder': 'Celular'})
...
def play():
    ...
    if request.method == 'POST' and form.validate():
        ...
        return render_template('play.html', play=products, form=form)
    if 'view' in request.args:
        ...
        if 'uid' in session:
            ...
            if result:
                ...
                mysql.connection.commit()
        return render_template('view_product.html', x=x, plays=product)
        ...
        product = curso.fetchall()
        x = content_based_filtering(product_id)
        return render_template('order_product.html', x=x, plays=product, form=form)
    return render_template('play.html', play=products, form=form)
...
def xbox():
    ...
    if 'view' in request.args:
        ...
        return render_template('view_product.html', x=x, plays=products)
    elif 'order' in request.args:
        product_id = request.args['order']
        curso = mysql.connection.cursor()
        curso.execute("SELECT * FROM products WHERE id=%s", (product_id,))
...
@app.route('/videojuegops4', methods=['GET', 'POST'])
def videojuegops4():
    form = OrderForm(request.form)
    # Create cursor
    cur = mysql.connection.cursor()
    ...
    if request.method == 'POST' and form.validate():
        ...
        flash('Pedido exitoso', 'success')
        return render_template('videojuegops4.html', videojuegops4=products, form=form)
    if 'view' in request.args:
        q = request.args['view']
        product_id = q
        x = content_based_filtering(product_id)
...
@app.route('/Videojuegoxbox', methods=['GET', 'POST'])
def Videojuegoxbox():
    form = OrderForm(request.form)
    # Create cursor
    cur = mysql.connection.cursor()
    ...
    if request.method == 'POST' and form.validate():
        ...
        flash('Pedido exitoso', 'success')
        return render_template('Videojuegoxbox.html', Videojuegoxbox=products, form=form)
    if 'view' in request.args:
        q = request.args['view']
        product_id = q
        x = content_based_filtering(product_id)
...
def ml():
    ...
    return render_template('pages/ml.html', result=result, row=num_rows, order_rows=order_rows,
                           users_rows=users_rows)
        ...
        if name and price and description and available and category and item and code and file:
            ...
            if picture.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                if save_photo:
                    ...
                    if category == 'play':
                        level = request.form.getlist('play')
                        for lev in level:
                            yes = 'yes'
                        ...
                        level = request.form.getlist('xbox')
                        for lev in level:
                            yes = 'yes'
                            query = 'UPDATE product_level SET {field}=%s WHERE product_id=%s'.format(field=lev)
                            curs.execute(query, (yes, product_id))
                            # Commit cursor
                            mysql.connection.commit()
                    elif category == 'videojuegops4':
                        level = request.form.getlist('videojuegops4')
                        for lev in level:
                            yes = 'yes'
                            query = 'UPDATE product_level SET {field}=%s WHERE product_id=%s'.format(field=lev)
                            curs.execute(query, (yes, product_id))
                            # Commit cursor
                            mysql.connection.commit()
                    elif category == 'Videojuegoxbox':
                        level = request.form.getlist('Videojuegoxbox')
                        for lev in level:
                            yes = 'yes'
                            query = 'UPDATE product_level SET {field}=%s WHERE product_id=%s'.format(field=lev)
                            curs.execute(query, (yes, product_id))
...
def search():
    ...
    if 'q' in request.args:
        ...
        # Close Connection
        cur.close()
        flash('Mostrando los resultados para: ' + q, 'success')
...
class UpdateRegisterForm(Form):
    name = StringField('Nombres', [validators.length(min=3, max=50)],
                       render_kw={'autofocus': True, 'placeholder': 'Nombres'})
    ...
    password = PasswordField('Contraseña', [validators.length(min=3)],
                             render_kw={'placeholder': 'Contraseña'})
    mobile = StringField('Celular', [validators.length(min=11, max=15)], render_kw={'placeholder': 'Celular'})
